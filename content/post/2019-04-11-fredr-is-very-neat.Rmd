...
```{r}
ChargeOff.Data <- map_dfr(ChargeOff$id, fredr)
```

Just to finish one piece, let me show what these data look like for one series.

```{r}
ChargeOff.Data %>% filter(series_id=="CORSREOBS") %>% 
...
SubPrime.OR %>% ggplot(aes(x=date, y=value, colour=series_id)) +
  ...
  labs(title='The Subprime Credit Rate Across Oregon Counties: {frame_along}', y ="Subprime Rate") +
...
OR_base + theme_minimal()
```

```{r}
counties <- map_data("county")
OR.county <- subset(counties, region == "oregon")
OR.Map <- OR_base + theme_minimal()  +
  geom_polygon(data = OR.county, fill = NA, color = "white") +
