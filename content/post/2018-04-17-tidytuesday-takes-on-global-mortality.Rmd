---
title: tidyTuesday takes on global mortality
author: RWW
date: '2018-04-17'
...
library(readxl)
library(skimr)
url <- "https://github.com/rfordatascience/tidytuesday/raw/master/data/global_mortality.xlsx"
destfile <- "global_mortality.xlsx"
curl::curl_download(url, destfile)
global_mortality <- read_excel(destfile)
skim(global_mortality)
```

```{r COrig}
library(tidyverse)
Counts <- read.csv("~/Downloads/annual-number-of-deaths-by-cause.csv")
...
ggplot.res <- ggplot.res[-1,]; ggplot.res <- ggplot.res[,16]
...
cplot <- ggplot(mydf, aes(name,value, fill=Emph)) + geom_bar(stat="identity") + coord_flip() + ylab("Within  Percent of Total Variation") + xlab("Cause of Mortality") + ggtitle("Within-country variation in the Causes of Death") + guides(fill="none")
cplot
```
