...
EPL.Contracts.df %>% group_by(Team) %>% summarise(Team.Mean=mean(value, na.rm=TRUE)/1e3) -> Team.mean
pp <- Team.mean %>% arrange(Team.Mean)
pp %>% ggplot(aes(Team.Mean,Team)) + geom_point() + labs(x="Avg. Salary (1000s)") -> cplot
cplot
```
