...
p <- dal_plt %>% ggplot() + aes(x = game_seconds_remaining, y = wpa, color = team, label = labs1) +
  ...
  ) + theme_dark() + theme(text=element_text(size=12)) + transition_states(titleS, transition_length=10, state_length = 15)
animate(p, nframes=450)
```


I can write a function to take an input and give an animation for a team.  The hardest part was figuring out the color map.  The first example, dedicated to Cristian Salgado that sparked my interest in these data, is San Francisco.  Then to try it out for another team, I will implement the Texans.
...
season_animator <- function(teamName, data1=full_season_2018, data2=all_2018_games, nflteams = nflscrapR::nflteams) {
  ...
  names(NFL.colors) <- nflteams$abbr
  teamName <- teamName
  title.ST <- nflscrapR::nflteams %>% filter(abbr==teamName) %>% select(team) %>% as.character()
  title.My <- paste0("2018 Season: ",title.ST)
  team_season <- data1 %>% filter(home_team==teamName | away_team==teamName)
  ...
  My.NFL.Table <- ntableH %>% inner_join(ntableA)
  My.NFL.Table <- My.NFL.Table %>% left_join(all_2018_games)
  Team.Table <- My.NFL.Table %>% filter(home_team==teamName | away_team==teamName) %>% select(game_id,Home_team,home_score, Away_team, away_score)
  team_wp <- team_season %>% 
  filter(!is.na(home_wp),!is.na(away_wp)) %>% 
    unite(GIDGSR, game_seconds_remaining, game_id, sep=":")
team_wp <- team_wp %>% dplyr::select(GIDGSR,
                home_wp,
                away_wp)  %>%
  gather(team, wpa, -GIDGSR) %>% separate(., GIDGSR, c("GSR", "game_id"), sep=":") %>% 
  mutate(game_seconds_remaining = as.integer(GSR))
  dal_plt <- team_wp %>% left_join(My.NFL.Table)
  ...
  p <- dal_plt %>% ggplot() + aes(x = game_seconds_remaining, y = wpa, color = team, label = labs1) +
  geom_line(size = 1) +
  geom_label(size=6.5, nudge_x = 50, hjust=1.5) +
  geom_hline(yintercept = 0.5, color = "gray", linetype = "dashed") +
  scale_color_manual(values = NFL.colors, guide=FALSE) +
#  scale_color_viridis_d(guide=FALSE) +
  ...
  labs(
    ...
    subtitle = "{closest_state}",
    caption = "Data from nflscrapR"
  ) + theme_dark() + theme(text=element_text(size=11)) + transition_states(titleS, transition_length=10, state_length = 15)
animate(p, nframes=450)
}
SFS <- season_animator("SF")
SFS
```
...
```{r, warning=FALSE, message=FALSE}
HOU <- season_animator("HOU")
HOU
```

I still have not completely figured out overtime.  That comes next.  `nflscrapR` is pretty cool.  My next plan for this one is a shiny app that allows us to select the abbreviation and show everything.