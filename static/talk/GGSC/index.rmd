...
.footnote[
[1] An install script for all the packages used is in the talk directory as `r paste0(here(),"PackagesUsed.R")`.  The interested reader can source `r paste0(here(),"XSCSetup.R")`.
]
...
Archigos %>% ggplot(aes(x=endYear, y=tenureY, color=entry, label=gender)) + 
  geom_text() +  theme(legend.text = element_text(size=8)) + 
  labs(x="Year Leader's Spell Ended", y="Tenure of Leader (not cumulative)", color="Entry Type", shape="Gender", title="Tenure and Year Spell Ended: geom_text", subtitle = "Entered : {ceiling(frame_time)}", size="Tenures?") -> p
panim <- p  + transition_time(startYear) + shadow_mark(alpha = 0.3, size = 0.75) #<<
animate(panim, nframes=300) #<<
```
