...
tseq <-function(x) {
tempd <- iisystem[x,]
data.frame(ccode = tempd$ccode, Name = as.character(tempd$CountryName), idacr=as.character(tempd$idacr), year=seq(tempd$IISstartYear, tempd$IISendYear, by=1))
}
CSTS <- lapply(seq(1,DI), function(x) { tseq(x) } )
CSTS.Data <- data.frame(do.call(rbind, CSTS))
CSTS.Data$Name <- as.character(CSTS.Data$Name)
...
```
readRDS(url(""https://github.com/robertwwalker/academic-mymod/raw/master/data/Archigos-etc/GWCSTS.rds""))
```
...
```{r ArchigosManip}
Archigos <- Archigos %>% group_by(leadid) %>% mutate(startYear = year(startdate), endYear = year(enddate)) %>% ungroup()
DI <- dim(Archigos)[[1]]
tseq <-function(x) {
  tempd <- Archigos[x,]
  data.frame(obsid = tempd$obsid, ccode = tempd$ccode, leader = as.character(tempd$leader), leadid = as.character(tempd$leadid), idacr=as.character(tempd$idacr), year=seq(tempd$startYear, tempd$endYear, by=1), LSY=tempd$startYear, LEY = tempd$endYear)
}
...
Full.LTS %>% group_by(leadid.x) %>% summarise(CC.sd = sd(ccode.x)) %>% arrange(desc(CC.sd)) %>% filter(CC.sd > 0)
# If countries are a proper nesting operator, then things can be made much easier.  They 
Full.LTS %>% filter(leadid.x == "81de4823-1e42-11e4-b4cd-db5882bf8def")
Full.LTS %>% filter(leadid.x == "821be4de-1e42-11e4-b4cd-db5882bf8def")
```
There are two leaders that led multiple countries.  The rest are nested.
...
```
readRDS(url(""https://github.com/robertwwalker/academic-mymod/raw/master/data/Archigos-etc/Archigos-LTS.rds""))
```
...
p <- BTSCSdata %>% filter(!is.na(posttenurefate)) %>% mutate(howEndedF = factor(posttenurefate)) %>% ggplot(aes(x=spell)) + geom_density(aes(color=howEndedF)) + scale_color_viridis_d(guide=FALSE)+ theme_minimal() + labs(title="How Long Do Leaders Last Given Fates?", x="Leader Duration in Years") 
p
```
