...
destfile <- "week6_coffee_chains.xlsx"
curl::curl_download(url, destfile)
Starbucks <- read_excel(destfile, sheet = "starbucks")
Dunkin.Donuts <- read_excel(destfile, sheet = "dunkin")
Tim.Hortons <- read_excel(destfile, sheet = "timhorton")
...
coffee.xl <- read_excel_allsheets("week6_coffee_chains.xlsx")
```
...
Tim.H <- coffee.xl$timhorton %>% filter(country=="us") %>% mutate_geocode(full.address)
...
USmap <-get_map(location='united states', zoom=4, maptype = "terrain", source='google',color='color')
mp <- ggmap(USmap)
mp <- mp + geom_point(aes(x=Starbucks$Longitude, y=Starbucks$Latitude) ,color="dark green", size=0.5) + xlab("") + ylab("")
mp <- mp + geom_point(aes(x=Dunkin.Donuts$loc_LONG_centroid, y=Dunkin.Donuts$loc_LAT_centroid) ,color="orange", size=0.5) + xlab("") + ylab("")
mp
