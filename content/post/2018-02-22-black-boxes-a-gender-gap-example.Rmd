...
OregonSalaries contains 32 observations: 16 males and 16 females.  The mean of all salaries is `r mean(OregonSalaries$Salary)`; let me put that in a plot in blue.  The total sum of squares can be represented as the sum of all the squared distances to the blue line; each vertical distance is demarcated with an arrow below in blue.  The total sum of squares: the total area of the black box in the original metric (squared dollars) is: `r sum(lm(Salary~1, data=OregonSalaries)$residuals^2)`.
...
What does the regression imply?  That salary for each individual $i$ is a function of a constant and gender.  Given the way that R works, $\alpha$ will represent the average for females and $\beta$ will represent the difference between male and female average salaries.

## A New Residual Sum of Squares

The picture now has a red line and a black line and the residual/leftover/unexplained sum of squares is now the difference between the point and the respective vertical line (red arrows or black arrows).  The sum of the remaining squared vertical distances is `r sum(lm(Salary~Gender, data=OregonSalaries)$residuals^2)`.  This is the amount leftover or residual given the gender.
