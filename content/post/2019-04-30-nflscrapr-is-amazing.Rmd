...
p <- dal_RP %>% ggplot(aes(x=game_date, y=count, color=play_type, size=2)) + geom_line() + scale_color_viridis_d() +  labs(title = "{frame_along}", y="Number of Plays", color="Play Type", x="Game Date") + theme_economist_white() + geom_point(aes(group=seq_along(game_date))) + transition_reveal(game_date) 
animate(p)
```
