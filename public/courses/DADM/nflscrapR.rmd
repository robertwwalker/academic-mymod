...
library(nflscrapR)
all_2018_games <-readRDS("../../../data/NFLGames2018.rds")
all_2018_games %>%
  pander::pander()
```
...
```{r}
full_season_2018 <- readRDS("../../../data/2018NFLSeason.rds") # Downloaded it once.  It sits in rww.science/data
```
...
The first thing to notice is that this color scheme thing is going to cause trouble when Dallas plays the Seahawks that apparently have the same primary color.  For now, I think that I will just make the opponent some constant color.
...
dal_wp <- dal_wp %>% dplyr::select(GIDGSR,
  ...
  gather(team, wpa, -GIDGSR) %>% separate(., GIDGSR, c("GSR", "game_id"), sep=":") %>% 
  mutate(game_seconds_remaining = as.integer(GSR))
dal_wp %>% ggplot(aes(x = game_seconds_remaining, y = wpa, color = team)) +
  geom_line(size = 2) +
  geom_hline(yintercept = 0.5, color = "gray", linetype = "dashed") +
  scale_color_viridis_d() +
#  scale_color_manual(labels = c("CAR", "DAL"),
#                     values = c("#7FFF00", dal_color),
#                     guide = FALSE) +
  scale_x_reverse(breaks = seq(0, 3600, 300)) + 
#  annotate("text", x = 3000, y = .75, label = "CAR", color = "#7FFF00", size = 8) + 
#  annotate("text", x = 3000, y = .25, label = "DAL", color = dal_color, size = 8) +
  geom_vline(xintercept = 900, linetype = "dashed", black) + 
  geom_vline(xintercept = 1800, linetype = "dashed", black) + 
  geom_vline(xintercept = 2700, linetype = "dashed", black) + 
