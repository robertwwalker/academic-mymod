...
OregonSalaries contains 32 observations: 16 males and 16 females.  The mean of all salaries is `r mean(OregonSalaries$Salary)`; let me put that in a plot.

```{r PlotG}
plot(y=OregonSalaries$Salary, x=c(1:32), ylab="Salary", col=as.factor(OregonSalaries$Gender), xlab="")
abline(h=mean(OregonSalaries$Salary), col="blue")
```

The total sum of squares: the total area of the black box in the original metric (squared dollars).  Here, it is the sum of the squared vertical distances to the mean line: `r sum(lm(Salary~1, data=OregonSalaries)$residuals^2)`.
...
The picture has a red line and a black line and the residual/leftover/unexplained sum of squares is now the difference between the point and the respective vertical line (red or black).  The sum of the remaining squared vertical distances is `r sum(lm(Salary~Gender, data=OregonSalaries)$residuals^2)`
