...
```{r}
library(readr)
USZCLL <- read_delim("../../data/us-zip-code-latitude-and-longitude.csv", ";", escape_double = FALSE, trim_ws = TRUE)
...
```{r}
library(leaflet)
map.l <- leaflet() %>% addTiles() %>% addCircles(., data=Plot.Zips, lng=~Longitude, lat=~Latitude, radius = ~Total/5000, popup = ~contact_zip)
map.l
```
